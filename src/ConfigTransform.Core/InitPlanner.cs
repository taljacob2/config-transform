...
/// resource list, environments, clients) -- docs/INIT_COMMAND_DESIGN.md "Manifest shape". Every
/// Environment layer lists every selected resource, deliberately with no `patch`
...
/// `init`'d tree resolve to nothing until `set` ran once per resource). Every Client layer
/// declares `extends` only, an empty `resources[]` -- the documented "layer that only exists to
/// declare `extends`" shape. Idempotent: re-running against a tree `init` or `set` already
/// touched merges in what's new (mirrors <see cref="SetTargetResolver.EnsureResourceListed"/>'s
/// convention) -- an existing `patch` reference is never cleared, an existing `resources[]` entry
/// is never duplicated, an already-set `extends` is never overwritten.
/// </summary>
public static class InitPlanner
{
    public static IReadOnlyList<InitFile> BuildPlan(
        string root, IReadOnlyList<string> resources, IReadOnlyList<string> environments, IReadOnlyList<string> clients)
    {
