...
| [`INIT_COMMAND_DESIGN.md`](INIT_COMMAND_DESIGN.md) | Design for `configtransform init` (designed, not implemented — see its status line): scaffolds a `.configtransform/` tree, scanning the repo for candidate resources, via an interactive form or a flag-driven quiet mode, plus a canned `--template` starter. |
