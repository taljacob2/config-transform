...
| [`MANIFEST_SCHEMA.md`](MANIFEST_SCHEMA.md) | The `manifest.json` input contract this tool consumes today — see `SELF_DESCRIBING_OVERLAYS_DESIGN.md` for the design that fully replaces it (decided, not yet implemented). |
| [`SELF_DESCRIBING_OVERLAYS_DESIGN.md`](SELF_DESCRIBING_OVERLAYS_DESIGN.md) | **Design fully decided, not yet implemented.** A Kustomize-style self-describing `configtransform.json` per layer directory, replacing the fixed base→Environments→Clients rule and `manifest.json` entirely. |
