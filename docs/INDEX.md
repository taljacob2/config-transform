...
| [`DIFF_LAYERS_DESIGN.md`](DIFF_LAYERS_DESIGN.md) | **Design only — not implemented.** A proposed `--diff-layers` flag that splits `--diff`'s output into one diff per layer that actually changed the resource, annotating a changed line with which earlier layer it overrides when relevant. |
