...
| [`HOST_LAYER_DESIGN.md`](HOST_LAYER_DESIGN.md) | **Design only, not implemented.** A third, optional `--host` layer axis under Client/Environment, for load-balanced production servers that need per-box config — read this before assuming `Client`/`Environment` are the only two layer dimensions this tool will ever have. |
